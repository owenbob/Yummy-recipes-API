...
def create_user():
    ...
    if user_info:
        ...
        if email_already_exists:
            return jsonify({"message":"This email has already been used to register"}),400
        
        if user_info["username"] == "" or user_info["email"]== "" or user_info["password"]== "":
            return jsonify({"message":"Please ensure you have input all your details"}),400

        if  not validate_email(user_info["email"]):
            return jsonify({"message":"Please input correct email"}),400
...
def login():
    ...
    if not user:
        return make_response("2.Could not verify because provided details are not for user"),400
...
def create_category(current_user):
    ...
    if data["category_title"]=="" or data["category_description"]== "":
        return jsonify({"message":"Please ensure that you have input a category title and description"}),400
...
def edit_category(current_user, category_id):
    ...
    if data["category_description"] == "":
        return jsonify({"message":"Please ensure that you have input a category description"}),400
    category.category_title = data["category_title"]
    category.category_description=data["category_description"]
    db.session.commit()

    return jsonify({"message" : "Category has been edited!"}),201
...
def create_recipe(current_user,category_id):
    ...
    if not  Available_category:
        return({"message":"Category not available"}),400
    if not request.json:
        return jsonify({"message ":"Invalid Data Submitted"}),400
...
def edit_recipe(current_user, recipe_id):
    ...
    return jsonify({"message" : "Recipe has been edited!"}),201
